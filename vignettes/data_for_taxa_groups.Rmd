...
Please follow the secure_the_api_key steps and secure the API in the R environment before retrieving the species traits form the database. To check on the available traits in the database, you can check with the **`fw_dbguide()`** or browse the database directly in [database traits](https://www.freshwaterecology.info/abbreviations.php).
